...
egalleymaker_cmd = os.path.join(shared_cfg.bkmkr_scripts_dir, "sectionstart_converter", "xml_docx_stylechecks", "rsuitevalidate_main.py")
######## ^^^^^ FIX THIS ^^^^^
    ...
    try:
        ...
        for fname in os.listdir(shared_cfg.parentdir):
            if os.path.splitext(fname)[1] == '.docx':
                ...
                popen_params = [shared_cfg.pypath, r'{}'.format(os.path.join(egalleymaker_cmd)), file, shared_cfg.runtype_string, \
